...
async fn main() -> Result<()> {
    ...
    let wallet_builder = MnemonicBuilder::<English>::default()
        .phrase(phrase)
        // Use this if your mnemonic is encrypted
        .password(password);

    let w1: LocalWallet = wallet_builder.clone().index(0u32)?.build()?;
    let w2: LocalWallet = wallet_builder.clone().index(1u32)?.build()?;
